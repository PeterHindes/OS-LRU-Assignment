...
#define LOOKAHEAD 3
    ...
    if (q[proc].active == 1)
    {
        ...
        for (int pagep = page; pagep < page+LOOKAHEAD && remainingProcesses > 0; pagep++)
        {
            if (q[proc].pages[pagep]) continue; // Avoid flaging already loaded memory as loading

            if (pagein(proc, pagep))
            { // If it becomes paged set the timestamp
                timestamps[proc][pagep] = tick;
            }
            else
            { // If it doesn't become paged then find the least recently used page and evict it!
                int minTime = INT_MAX;
                int minProc = -1;
                int minPage = -1;
                for (int lproc = 0; lproc < MAXPROCESSES; lproc++)
                {
                    ...
                    {
                        if (q[lproc].pages[lpage] == 1 && minTime > timestamps[lproc][lpage])
                        { // only interested in pages that are in memory and with a lower timestamp
                ...
                { // Might happen if everything is currently swapping in.
