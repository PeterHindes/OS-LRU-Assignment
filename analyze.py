...
def run_test_predict():
    ...
    print("Raw output from ./test-predict:")
...
def thread_run_test(thread_id, runs, result_queue, start_delay=0):
    ...
    for i in range(runs):
        ...
        with run_lock:
            # Ensure 1.5 seconds have passed since the last execution
            current_time = time.time()
            if current_time - last_execution_time < 1.5:
                wait_time = 1.5 - (current_time - last_execution_time)
                print(f"Thread {thread_id} waiting {wait_time:.2f}s before starting run {i+1}")
            ...
            mean, stddev, max_swing = compute_statistics(ratio_list)
            print(f"  Thread {thread_id} - Ratio - Mean: {mean:.6f}, Std Dev: {stddev:.6f}, Max Swing: {max_swing:.6f}")
            print(f"  Thread {thread_id} - Ratio - Max: {max(ratio_list):.6f}")
            print("--------------------------------------------------")
        
        # Wait 1.5 seconds before next run to ensure no runs start within 1.5s of each other
        time.sleep(1.5)
...
def save_statistics(label, total_runs, stats_dict):
    ...
    with open(csv_file, 'a', newline='') as f:
        fieldnames = ['timestamp', 'label', 'total_runs', 
                      'mean_ratio', 'stddev_ratio', 'max_swing_ratio', 'max_ratio']
...
def main():
    ...
    runs = 8  # Number of times to run the test for each thread
    num_threads = 7  # Number of threads to run
    stagger_time = 1.5  # Time in seconds between thread starts
    ...
    mean, stddev, max_swing = compute_statistics(all_ratio)
