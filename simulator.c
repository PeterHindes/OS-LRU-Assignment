...
{
	if (process < 0 || process >= procs || !processes[process] || !processes[process]->active || page < 0 || page >= processes[process]->npages)
		return FALSE;
	...
	for (i = 0; i < procs; i++)
	{
		if (processes[i] && processes[i]->active)
		{
			...
			for (j = 0; j < processes[i]->npages; j++)
			{
				...
				else if (processes[i]->pages[j] < 0 && processes[i]->pages[j] >= -PAGEWAIT)
				{
					processes[i]->pages[j]--;
